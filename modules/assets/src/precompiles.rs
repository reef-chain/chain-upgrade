    ...
    alloy::primitives::U256: TryFrom<<Runtime as Config<Instance>>::Balance>,
{
    ...
    fn balance_of(
        ...
        let value = Self::to_u256(crate::Pallet::<Runtime, Instance>::balance(
            asset_id, account,
        ))?;
...
mod test {
    ...
    fn assert_contract_event(contract: H160, event: IERC20Events) {
        let (topics, data) = event.into_log_data().split();
        let topics = topics.into_iter().map(|v| H256(v.0)).collect::<Vec<_>>();
        System::assert_has_event(RuntimeEvent::Revive(
    ...
    fn balance_of_works() {
        new_test_ext().execute_with(|| {
            ...
            let account = <Test as pallet_revive::Config>::AddressMapper::to_address(&1)
                .0
                .into();
