...
fn calling_dead_account_fails_if_freezes_or_balances_on_hold_exist_2() {
    new_test_ext().execute_with(|| {
        ...
        let touch_deposit = account
            .reason
            .take_deposit()
            .expect("account was created by touching it; qed");
...
fn weights_sane() {
    ...
    assert_eq!(
        <() as crate::WeightInfo>::finish_destroy(),
        info.call_weight
    );
}
...
fn increasing_or_decreasing_destroying_asset_should_not_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Assets::can_withdraw(0, &1, 10),
            WithdrawConsequence::<_>::Success
        );
        assert_eq!(
            Assets::can_increase(0, &1, 10, false),
            DepositConsequence::Success
        );
        assert_eq!(
            Assets::can_decrease(0, &1, 10, false),
            WithdrawConsequence::<_>::Success
        );
...
fn asset_id_cannot_be_reused() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Assets::create(RuntimeOrigin::signed(1), 1, 1, 1),
            Error::<Test>::BadAssetId
        );
        assert_noop!(
        ...
        assert_noop!(
            Assets::create(RuntimeOrigin::signed(1), 5, 1, 1),
            Error::<Test>::BadAssetId
        );
