...
pub use evm::{Context, ExitError, ExitFatal, ExitRevert, ExitSucceed};
...
use support::{
    EVMStateRentTrait, ExecutionMode, InvokeContext, TransactionPayment, EVM as EVMTrait,
};
...
pub mod module {
    ...
    pub trait Config: frame_system::Config + pallet_timestamp::Config {
        ...
        type FreeDeploymentOrigin: EnsureOrigin<Self::RuntimeOrigin>;
    ...
    impl<T: Config> EvmAccountInfo<T> {
        pub fn new(nonce: T::Nonce, contract_info: Option<ContractInfo>) -> Self {
            Self {
                nonce,
                contract_info,
    ...
    pub type Accounts<T: Config> =
        StorageMap<_, Twox64Concat, EvmAddress, EvmAccountInfo<T>, OptionQuery>;
    ...
    #[pallet::type_value]
    pub fn EmptyEventVec<T: Config>() -> Vec<Event<T>> {
        Vec::new()
    }
        ...
        fn build(&self) {
            self.accounts.iter().for_each(|(address, account)| {
                ...
                if !account.code.is_empty() {
                    ...
                    <Pallet<T>>::on_contract_initialization(
                        address,
                        &EvmAddress::default(),
                        account.code.clone(),
                        used_gas_and_storage,
                    )
                    .expect("Genesis contract shouldn't fail");

                    #[cfg(not(feature = "with-ethereum-compatibility"))]
                    <Pallet<T>>::mark_deployed(*address, None)
                        .expect("Genesis contract shouldn't fail");
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn call(
            ...
            if info.exit_reason.is_succeed() {
                ...
                Self::deposit_event(Event::<T>::ExecutedFailed(
                    source,
                    target,
                    info.exit_reason,
                    info.output,
        ...
        pub fn create(
            ...
            let info =
                Runner::<T>::create(source, init, value, gas_limit, storage_limit, T::config())?;
        ...
        pub fn create_network_contract(
            ...
            let info = Runner::<T>::create_at_address(
                source,
                init,
                value,
                address,
            ...
            if !info.exit_reason.is_succeed() {
                Self::deposit_event(Event::<T>::CreatedFailed(
                    source,
                    info.address,
                    info.exit_reason,
                    info.output,
        ...
        pub fn deploy(origin: OriginFor<T>, contract: EvmAddress) -> DispatchResultWithPostInfo {
            ...
            Self::deposit_event(Event::<T>::ContractDeployed(contract));
            Ok(().into())
        }
        ...
        pub fn disable_contract_development(origin: OriginFor<T>) -> DispatchResultWithPostInfo {
            ...
            let address =
                T::AddressMapping::get_evm_address(&who).ok_or(Error::<T>::AddressNotMapped)?;
            let deposit = Accounts::<T>::mutate(
                address,
                |maybe_account_info| -> Result<BalanceOf<T>, Error<T>> {
...
impl<T: Config> Pallet<T> {
    ...
    pub fn account_basic(address: &EvmAddress) -> Account {
        ...
        let nonce =
            Self::accounts(address).map_or(Default::default(), |account_info| account_info.nonce);
    ...
    pub fn on_contract_initialization(
        ...
        Accounts::<T>::mutate(address, |maybe_account_info| {
            if let Some(account_info) = maybe_account_info.as_mut() {
                ...
                let account_info =
                    EvmAccountInfo::<T>::new(Default::default(), Some(contract_info.clone()));
                *maybe_account_info = Some(account_info);
            }
        });

        QueuedEvents::<T>::mutate(|v| {
            v.push(Event::<T>::Created(
                *maintainer,
                *address,
                used_gas_and_storage,
            ))
    ...
    fn do_selfdestruct(caller: &EvmAddress, contract: EvmAddress) -> DispatchResult {
        ...
        let contract_info = account_info
            ...
            contract_info.maintainer == *caller,
            Error::<T>::NoPermission
        );
        ensure!(!contract_info.deployed, Error::<T>::ContractAlreadyDeployed);
...
impl<T: Config> EVMStateRentTrait<T::AccountId, BalanceOf<T>> for Pallet<T> {
    ...
    fn query_maintainer(contract: EvmAddress) -> Result<EvmAddress, DispatchError> {
        Accounts::<T>::get(contract).map_or(
            Err(Error::<T>::ContractNotFound.into()),
            |account_info| {
                account_info
                    .contract_info
                    .map_or(Err(Error::<T>::ContractNotFound.into()), |v| {
                        Ok(v.maintainer)
                    })
            },
        )
