...
impl<'vicinity, 'config, T: Config> Handler<'vicinity, 'config, '_, T> {
    pub fn run_transaction<
        R,
        F: FnOnce(&mut Handler<'vicinity, 'config, '_, T>) -> TransactionOutcome<R>,
    >(
        ...
        frame_support::storage::with_transaction(|| {
            ...
            let storage_meter =
                match StorageMeter::new(&mut storage_meter_handler, contract, storage_limit) {
                    Ok(x) => x,
    ...
    pub fn run_sub_transaction<
        ...
        R,
        F: FnOnce(
            &mut Handler<'vicinity, 'config, '_, T>,
            &mut Gasometer<'_>,
...
impl<'vicinity, 'config, 'meter, T: Config> HandlerT for Handler<'vicinity, 'config, 'meter, T> {
    ...
    fn mark_delete(&mut self, address: H160, target: H160) -> Result<(), ExitError> {
        ...
        T::TransferAll::transfer_all(&source, &dest)
            .map_err(|_| ExitError::Other("TransferAllError".into()))?;
    ...
    fn create(
        &mut self,
        caller: H160,
        scheme: evm_runtime::CreateScheme,
        value: U256,
        init_code: Vec<u8>,
                ...
                match reason {
                    ExitReason::Succeed(s) => match substate.gasometer.record_deposit(out.len()) {
                        Ok(()) => {
                            ...
                            try_or_rollback!(
                                gasometer.record_refund(substate.gasometer.refunded_gas())
                            );

                            Handler::<T>::inc_nonce(address);
                            let storage_used =
                                (out.len() as u32).saturating_add(T::NewContractExtraBytes::get());
                            ...
                            match <Pallet<T>>::on_contract_initialization(
                                &address,
                                origin,
                                out,
                                used_gas_and_storage,
                            ) {
                                Ok(()) => TransactionOutcome::Commit(Capture::Exit((
                                    s.into(),
                                    Some(address),
                                    Vec::new(),
                                ))),
                    ...
                    ExitReason::Revert(r) => {
                        TransactionOutcome::Rollback(Capture::Exit((r.into(), None, out)))
                    }
                    ExitReason::Error(e) => {
    ...
    fn call(
        &mut self,
        code_address: H160,
        transfer: Option<evm_runtime::Transfer>,
        input: Vec<u8>,
        target_gas: Option<u64>,
        is_static: bool,
        context: Context,
