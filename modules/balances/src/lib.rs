...
pub mod pallet {
    ...
    pub enum Event<T: Config<I>, I: 'static = ()> {
        ...
        Held {
            reason: <T as pallet::Config<I>>::RuntimeHoldReason,
            who: T::AccountId,
            amount: T::Balance,
        },
    ...
    pub enum Error<T, I = ()> {
        ...
        TooManyReserves,
        /// Number of holds exceed `VariantCountOf<<T as pallet::Config<I>>::RuntimeHoldReason>`.
        TooManyHolds,
    ...
    pub type Holds<T: Config<I>, I: 'static = ()> = StorageMap<
        ...
        T::AccountId,
        BoundedVec<
            IdAmount<<T as pallet::Config<I>>::RuntimeHoldReason, T::Balance>,
            VariantCountOf<<T as pallet::Config<I>>::RuntimeHoldReason>,
        >,
        ValueQuery,
    >;
    ...
    impl<T: Config<I>, I: 'static> Pallet<T, I> {
        ...
        #[pallet::call_index(6)]
        #[pallet::weight(<T as pallet::Config<I>>::WeightInfo::upgrade_accounts(who.len() as u32))]
        ...
        #[pallet::call_index(8)]
        #[pallet::weight(
			<T as pallet::Config<I>>::WeightInfo::force_set_balance_creating() // Creates a new account.
				.max(<T as pallet::Config<I>>::WeightInfo::force_set_balance_killing()) // Kills an existing account.
		)]
        ...
        #[pallet::call_index(9)]
        #[pallet::weight(<T as pallet::Config<I>>::WeightInfo::force_adjust_total_issuance())]
        ...
        #[pallet::weight(if *keep_alive {<T as pallet::Config<I>>::WeightInfo::burn_allow_death() } else {<T as pallet::Config<I>>::WeightInfo::burn_keep_alive()})]
        pub fn burn(
            origin: OriginFor<T>,
            #[pallet::compact] value: T::Balance,
            keep_alive: bool,
    ...
    impl<T: Config<I>, I: 'static> Pallet<T, I> {
        ...
        pub(crate) fn mutate_account<R>(
            ...
            Self::try_mutate_account(
                who,
                force_consumer_bump,
                |a, _| -> Result<R, DispatchError> { Ok(f(a)) },
            )
        }
        ...
        pub fn derive_dev_account(num_accounts: u32, balance: T::Balance, derivation: &str) {
            ...
            for index in 0..num_accounts {
                ...
                let pair: SrPair = Pair::from_string(&derivation_string, None).expect(&format!(
                    "Failed to parse derivation string: {derivation_string}"
                ));

                // Convert the public key to AccountId.
                let who = T::AccountId::decode(&mut &pair.public().encode()[..]).expect(&format!(
                    "Failed to decode public key from pair: {:?}",
                    pair.public()
                ));
        ...
        fn hold_and_freeze_count() -> Result<(), sp_runtime::TryRuntimeError> {
            Holds::<T, I>::iter_keys().try_for_each(|k| {
                if Holds::<T, I>::decode_len(k).unwrap_or(0)
                    > <T as pallet::Config<I>>::RuntimeHoldReason::VARIANT_COUNT as usize
                {
