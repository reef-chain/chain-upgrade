   ...
   #[pallet::generate_deposit(pub(crate) fn deposit_event)]
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn claim_account(
            ...
            let address = Self::eth_recover(
                &eth_signature,
                &who.using_encoded(Self::convert_to_ascii_hex)?,
                &[][..],
            )?;
...
impl<T: Config> Pallet<T> {
    ...
    pub fn ethereum_signable_message(what: &[u8], extra: &[u8]) -> Result<Vec<u8>, DispatchError> {
        ...
        let mut l = prefix
            ...
            .and_then(|sum| sum.checked_add(extra.len()))
    ...
    pub fn eth_sign(
        ...
        what: &[u8],
        extra: &[u8],
    ) -> Result<Option<EcdsaSignature>, DispatchError> {
    ...
    pub fn convert_to_ascii_hex(data: &[u8]) -> Result<Vec<u8>, DispatchError> {
        let len = data.len().checked_mul(2).ok_or(Error::<T>::NumOverflow)?;
            ...
            Error::<T>::VecInsufficientCapacity
        );
    ...
    pub fn push_nibble(nibble: u8, r: &mut Vec<u8>) -> Result<(), DispatchError> {
        let value = (if nibble < 10 {
            b'0'.checked_add(nibble)
        } else {
            b'a'.checked_sub(10).and_then(|sum| sum.checked_add(nibble))
        })
        .ok_or(Error::<T>::NumOverflow)?;
        r.push(value);
        Ok(())
