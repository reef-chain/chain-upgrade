...
pub fn run_primitive<P: PrimitivePrecompile<T = Test>>(input: Vec<u8>) -> Result<Vec<u8>, Error> {
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert!(
            P::MATCHER.is_fixed(),
            "All pre-compiles we are testing here are fixed"
        );
...
pub fn run_failure_test_vectors<P: PrimitivePrecompile<T = Test>>(json: &str) {
    ...
    for test in tests {
        ...
        match run_primitive::<P>(input) {
            Err(Error::Error(ExecError {
                error: DispatchError::Other(reason),
                ..
            })) => {
                assert_eq!(
...
fn public_matching_works() {
    ...
    assert_eq!(
        matcher_fixed.base_address(),
        hex!("0000000000000000000000000000000000420000")
    );
    assert_eq!(
        matcher_fixed.base_address(),
        matcher_fixed.highest_address()
    );

    assert_eq!(
        matcher_prefix.base_address(),
        hex!("0000000000000000000000000000000000080000")
    );
    assert_eq!(
        matcher_prefix.highest_address(),
        hex!("FFFFFFFF00000000000000000000000000080000")
    );
}
