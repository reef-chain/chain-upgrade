    ...
    fn call_erc20_contract() {
        ExtBuilder::default()
            ...
            .execute_with(|| {
                ...
                let balance = EU256::abi_decode_validate(&result.data)
    ...
    fn get_balance_of_erc20() {
        ExtBuilder::default()
            ...
            .execute_with(|| {
                ...
                .build_and_unwrap_contract();
                assert_eq!(
                    <Contracts as fungibles::Inspect<_>>::balance(addr, &ALICE),
                    amount
                );
            });
    }
        ...
        ExtBuilder::default()
            ...
            .execute_with(|| {
                ...
                let constructor_data =
                    sol_data::Uint::<256>::abi_encode(&(EU256::from(amount * 2)));
                let Contract { addr, .. } = BareInstantiateBuilder::<Test>::bare_instantiate(
                    RuntimeOrigin::signed(ALICE),
                    Code::Upload(code),
                )
    ...
    fn mint_into_impl_works() {
        ExtBuilder::default()
            ...
            .execute_with(|| {
                ...
                assert_ok!(<Contracts as fungibles::Mutate<_>>::mint_into(
                    addr, &ALICE, amount
                ));
