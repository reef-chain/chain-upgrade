...
pub mod pallet {
    ...
    use pallet_balances::GetTotalIssuance;
    use sp_core::U256;
    ...
    pub trait Config: frame_system::Config  {
        ...
        #[pallet::no_default]
        type Issuance : GetTotalIssuance;
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        ...
        fn on_finalize(block_number: BlockNumberFor<T>) {
            let total_issuance = T::Issuance::total_issuance();
            TotalReviveIssuance::<T>::put(total_issuance);
...
impl<T: Config> Pallet<T> {
            ...
            let executable = match code {
                ...
                Code::Existing(code_hash) => {
                    let executable = ContractBlob::from_storage(code_hash, &mut transaction_meter)?;
                        ...
                        executable.code_info().is_pvm(),
                        <Error<T>>::EvmConstructedFromHash
                    );
                    executable
                }
            };
        ...
        CallOf<T>: SetWeightLimit,
    {
        ...
        let extract_error = |err| {
            if err == Error::<T>::StorageDepositNotEnoughFunds.into() {
                Err(EthTransactError::Message(format!(
                    "Not enough gas supplied: {err:?}"
                )))
            } else {
                Err(EthTransactError::Message(format!(
                    "failed to run contract: {err:?}"
                )))
            }
        };
        ...
        let mut dry_run = match to {
            ...
            Some(dest) => {
                if dest == RUNTIME_PALLETS_ADDR {
                    ...
                    if let Err(result) = dispatch_call
                        .clone()
                        .dispatch(RawOrigin::Signed(origin).into())
                    {
                        return Err(EthTransactError::Message(format!(
    ...
    fn refund_deposit(
        ...
        let result = if exec_config
            .map(|c| c.collect_deposit_from_hold.is_some())
            .unwrap_or(false)
        {
...
pub const RUNTIME_PALLETS_ADDR: H160 = H160(hex_literal::hex!(
    "6d6f646c70792f70616464720000000000000000"
));
