...
pub mod pallet {
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        ...
        fn integrity_test() {
            ...
            let max_call_depth = limits::CALL_STACK_DEPTH
                .checked_add(1)
    ...
    pub fn bare_instantiate(
        ...
        let try_instantiate = || {
            ...
            let (executable, upload_deposit) = match code {
                ...
                Code::Existing(code_hash) => (
                    WasmBlob::from_storage(code_hash, &mut gas_meter)?,
                    Default::default(),
                ),
            };
    ...
    fn convert_evm_to_native(
        ...
        match (precision, remainder.is_zero()) {
            (ConversionPrecision::Exact, false) => {
                log::error!(
                    target: LOG_TARGET,
                    "❌ Decimal precision loss in conversion (value = {:?}, ratio = {:?})",
                    value,
                    ratio
                );
            ...
            (_, true) => {
                let result: BalanceOf<T> = quotient.try_into().map_err(|_| {
                    log::error!(
                        target: LOG_TARGET,
                        "❌ Failed to convert quotient {:?} to BalanceOf<T>",
                        quotient
                    );
            ...
            (_, false) => {
                let rounded = quotient.saturating_add(U256::one());
                let result: BalanceOf<T> = rounded.try_into().map_err(|_| {
                    log::error!(
                        target: LOG_TARGET,
                        "❌ Failed to convert rounded quotient {:?} to BalanceOf<T>",
                        rounded
                    );
                    Error::<T>::BalanceConversionFailed
                })?;
