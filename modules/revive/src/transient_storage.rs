...
impl<T: Config> StorageMeter<T> {
    ...
    fn new(memory_limit: u32) -> Self {
        Self {
            root_meter: MeterEntry::new(memory_limit),
            ..Default::default()
        }
    }
    ...
    fn total_amount(&self) -> u32 {
        self.nested_meters
            .iter()
            .fold(self.root_meter.amount, |acc, e| {
                acc.saturating_add(e.amount)
            })
    }
...
impl<T: Config> TransientStorage<T> {
    ...
    pub fn read(&self, account: &AccountIdOf<T>, key: &Key) -> Option<Vec<u8>> {
        self.storage
            .read(&Self::storage_key(&account.encode(), &key.hash()))
    }
    ...
    pub fn write(
        ...
        if prev_value != value {
            ...
            // Update the journal.
            self.journal
                .push(JournalEntry::new(key, prev_value.clone()));
        }
...
mod tests {
    ...
    fn metering_transactions_works() {
        ...
        assert_eq!(
            storage.meter().current().limit - storage.meter().current().amount,
            size
        );
        assert_eq!(
