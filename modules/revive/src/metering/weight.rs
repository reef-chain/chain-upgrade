...
impl<T: Config> EngineMeter<T> {
    ...
    fn set_fuel(&mut self, fuel: u64) -> Weight {
        let consumed = self
            .fuel
            .saturating_sub(fuel)
            .saturating_mul(Self::ref_time_per_fuel());
        self.fuel = fuel;
...
impl<T: Config> WeightMeter<T> {
    ...
    pub fn charge_or_halt<Tok: Token<T>>(
        ...
        self.charge(token).map_or_else(
            |_| ControlFlow::Break(Error::<T>::OutOfGas.into()),
    ...
    pub fn sync_to_executor(&mut self) -> polkavm::Gas {
        self.engine_meter
            .sync_remaining_ref_time(self.weight_left().ref_time())
    }
    ...
    pub fn weight_left(&self) -> Weight {
        self.effective_weight_limit
            .saturating_sub(self.weight_consumed)
    }
