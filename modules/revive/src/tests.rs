...
pub mod test_utils {
    ...
    pub fn lockup_deposit(code_hash: &sp_core::H256) -> BalanceOf<Test> {
        CodeHashLockupDepositPercent::get()
            .mul_ceil(get_code_deposit(code_hash))
            .into()
    }
...
impl ExtBuilder {
    ...
    pub fn build(self) -> sp_io::TestExternalities {
        ...
        ext.execute_with(|| {
            assert_ok!(Pallet::<Test>::map_account(RuntimeOrigin::signed(
                checking_account
            )));
        });
        ext
        ...
        .execute_with(|| {
            ...
            for nonce in 1..3 {
                ...
                assert_eq!(
                    addr,
                    create1(
                        &<Test as Config>::AddressMapper::to_address(&ALICE),
                        nonce - 1
                    )
                );
            }
        ...
        .execute_with(|| {
            ...
            assert_ok!(builder::call(caller_addr)
                .data(
                    (
                        callee_code_hash,
                        code_load_weight.ref_time(),
                        code_load_weight.proof_size()
                    )
                        .encode()
                )
                .build());
...
fn cannot_self_destruct_through_storage_refund_after_price_change() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            // one unit to little
            assert_eq!(
                get_contract(&contract.addr).extra_deposit(),
                16 + 32 + 2 + 1
            );
        });
}
...
fn lazy_removal_does_not_use_all_weight() {
    ...
    ext.execute_with(|| {
        ...
        assert_eq!(
            meter.consumed(),
            weight_per_key.mul(vals.len() as _) + base_weight
        );
...
fn contract_reverted() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            // Calling extrinsic: revert leads to an error
            assert_err_ignore_postinfo!(
                builder::instantiate_with_code(wasm)
                    .data(input.clone())
                    .build(),
                <Error<Test>>::ContractReverted,
            );
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            let callee_min_deposit = {
                let callee_info_len =
                    ContractInfoOf::<Test>::get(&addr).unwrap().encoded_size() as u64;
            ...
            // origin.
            assert_eq!(
                <Test as Config>::Currency::free_balance(&callee_account_id),
                10_000 + ED
            );
...
fn native_dependency_deposit_works() {
    ...
    for code in [Code::Upload(wasm.clone()), Code::Existing(code_hash)] {
        ExtBuilder::default().build().execute_with(|| {
            ...
            let extra_deposit = add_upload_deposit
                .then(|| upload_deposit)
                .unwrap_or_default();
...
fn block_hash_works() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_ok!(builder::call(addr)
                .data((U256::from(1), H256::zero()).encode())
                .build());
        });
}
...
fn gas_consumed_is_linear_for_nested_calls() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_eq!(
                gas_max,
                gas_0 + gas_per_recursion * limits::CALL_STACK_DEPTH as u64
            );
        });
}
...
fn read_only_call_cannot_store() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_err_ignore_postinfo!(
                builder::call(addr_caller)
                    .data((&addr_callee, 100u32).encode())
                    .build(),
                <Error<Test>>::ContractTrapped
            );
        });
            ...
            assert_err_ignore_postinfo!(
                builder::call(addr_caller)
                    .data(
                        (
                            addr_callee,
                            pallet_revive_uapi::CallFlags::READ_ONLY.bits(),
                            100u64
                        )
                            .encode()
                    )
...
fn read_only_subsequent_call_cannot_store() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            let input = (
                &addr_callee,
                pallet_revive_uapi::CallFlags::empty().bits(),
                0u64,
                100u32,
            );
...
fn call_data_size_api_works() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            let received = builder::bare_call(addr)
                .data(vec![1; 256])
                .build_and_unwrap_result();
            assert_eq!(received.flags, ReturnFlags::empty());
...
fn immutable_data_works() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            let account = <Test as Config>::AddressMapper::to_account_id(&addr);
            let actual_deposit = test_utils::get_balance_on_hold(
                &HoldReason::StorageDepositReserve.into(),
                &account,
            );
...
fn to_account_id_works() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            let expected_mapped_account_id =
...
fn code_hash_works() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_ok!(builder::call(addr)
                .data((dummy_addr, code_hash).encode())
                .build());
            // code has of itself
            assert_ok!(builder::call(addr)
                .data((addr, self_code_hash).encode())
                .build());
            ...
            assert_ok!(builder::call(addr)
                .data((BOB_ADDR, H256::zero()).encode())
                .build());
...
fn recovery_works() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            <Pallet<Test>>::dispatch_as_fallback_account(
...
fn precompiles_work() {
    ...
    let cases = vec![
        (
            INoInfo::INoInfoCalls::identity(INoInfo::identityCall {
                number: 42u64.into(),
            })
            .abi_encode(),
            42u64.abi_encode(),
        ...
        (
            INoInfo::INoInfoCalls::reverts(INoInfo::revertsCall {
