...
pub const EMPTY_CODE_HASH: H256 = H256(sp_core::hex2array!(
...
pub trait PrecompileExt: sealing::Sealed {
    ...
    fn charge(&mut self, weight: Weight) -> Result<crate::gas::ChargedAmount, DispatchError> {
        self.gas_meter_mut()
            .charge(crate::RuntimeCosts::Precompile(weight))
    }
...
enum ExecutableOrPrecompile<T: Config, E: Executable<T>, Env> {
    ...
    Precompile {
        instance: PrecompileInstance<Env>,
    ...
    T::Hash: frame_support::traits::IsType<H256>,
{
    ...
    fn push_frame(
        ...
        if let Some((frame, executable)) = Self::new_frame(
            ...
            self.frames
                .try_push(frame)
                .map_err(|_| Error::<T>::MaxCallDepthReached)?;
            Ok(Some(executable))
        } else {
            Ok(None)
        }
    ...
    fn run(
        ...
        if let Some(caller_frame) = match frames_len {
            ...
            _ => self
                .frames
                .get_mut(frames_len - 2)
                .map(|frame| &mut frame.last_frame_output),
        } {
    ...
    fn pop_frame(&mut self, persist: bool) {
        ...
        if let Some(mut frame) = frame {
            ...
            prev.nested_storage
                .absorb(frame.nested_storage, account_id, contract.as_mut());
    ...
    fn frames(&self) -> impl Iterator<Item = &Frame<T>> {
        core::iter::once(&self.first_frame)
            .chain(&self.frames)
            .rev()
    }
    ...
    fn frames_mut(&mut self) -> impl Iterator<Item = &mut Frame<T>> {
        core::iter::once(&mut self.first_frame)
    ...
    T::Hash: frame_support::traits::IsType<H256>,
{
    ...
    fn set_code_hash(&mut self, hash: H256) -> DispatchResult {
        ...
        frame
            .nested_storage
            .charge_deposit(frame.account_id.clone(), deposit);
    ...
    fn set_immutable_data(&mut self, data: ImmutableData) -> Result<(), DispatchError> {
        ...
        frame
            .contract_info()
            .set_immutable_data_len(data.len() as u32);
    ...
    T::Hash: frame_support::traits::IsType<H256>,
{
    ...
    fn instantiate(
        ...
        *self.last_frame_output_mut() = Default::default();
        ...
        self.run(
            executable.expect(FRAME_ALWAYS_EXISTS_ON_INSTANTIATE),
            input_data,
        )
        .map(|_| address)
    }
    ...
    T::Hash: frame_support::traits::IsType<H256>,
{
    ...
    fn set_transient_storage(
        ...
        let account_id = self.account_id().clone();
        self.transient_storage
            .write(&account_id, key, value, take_old)
    }
