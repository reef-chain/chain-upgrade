...
fn transfer_works() {
    ...
    ExtBuilder::default().build().execute_with(|| {
        ...
        assert_eq!(
            storage_meter
                .try_into_deposit(&Origin::from_account_id(ALICE), false)
                .unwrap(),
...
fn caller_returns_proper_values() {
    ...
    let bob_ch = MockLoader::insert(Call, |ctx, _| {
        ...
        WitnessedCallerBob::mutate(|caller| {
            ...
            *caller = Some(
                <<Test as Config>::AddressMapper as AddressMapper<Test>>::to_address(
...
fn code_hash_returns_proper_values() {
    let bob_code_hash = MockLoader::insert(Call, |ctx, _| {
        ...
        assert_eq!(
            ctx.ext.code_hash(&BOB_ADDR),
            H256(keccak_256(&0u64.to_le_bytes()))
        );
...
fn get_storage_varsized_key_works() {
    let code_hash = MockLoader::insert(Call, |ctx, _| {
        ...
        assert_eq!(
            ctx.ext
                .get_storage(&Key::try_from_var([1; 19].to_vec()).unwrap()),
            Some(vec![1, 2, 3])
        );
        assert_eq!(
            ctx.ext
                .get_storage(&Key::try_from_var([2; 16].to_vec()).unwrap()),
            Some(vec![])
        );
        assert_eq!(
            ctx.ext
                .get_storage(&Key::try_from_var([3; 8].to_vec()).unwrap()),
            None
        );
...
fn get_storage_size_varsized_key_works() {
    let code_hash = MockLoader::insert(Call, |ctx, _| {
        ...
        assert_eq!(
            ctx.ext
                .get_storage_size(&Key::try_from_var([1; 19].to_vec()).unwrap()),
            Some(3)
        );
        assert_eq!(
            ctx.ext
                .get_storage_size(&Key::try_from_var([2; 16].to_vec()).unwrap()),
            Some(0)
        );
        assert_eq!(
            ctx.ext
                .get_storage_size(&Key::try_from_var([3; 8].to_vec()).unwrap()),
            None
        );
...
fn set_transient_storage_works() {
    let code_hash = MockLoader::insert(Call, |ctx, _| {
        ...
        assert_eq!(
            ctx.ext
                .set_transient_storage(&Key::Fix([1; 32]), Some(vec![1, 2, 3]), false),
            Ok(WriteOutcome::New)
        );
        assert_eq!(
            ctx.ext
                .set_transient_storage(&Key::Fix([2; 32]), Some(vec![4, 5, 6]), true),
            Ok(WriteOutcome::New)
        );
        assert_eq!(
            ctx.ext
                .set_transient_storage(&Key::Fix([3; 32]), None, false),
            Ok(WriteOutcome::New)
        );
        assert_eq!(
            ctx.ext
                .set_transient_storage(&Key::Fix([4; 32]), None, true),
            Ok(WriteOutcome::New)
        );
        assert_eq!(
            ctx.ext
                .set_transient_storage(&Key::Fix([5; 32]), Some(vec![]), false),
            Ok(WriteOutcome::New)
        );
        assert_eq!(
            ctx.ext
                .set_transient_storage(&Key::Fix([6; 32]), Some(vec![]), true),
            Ok(WriteOutcome::New)
        );
        ...
        assert_eq!(
            ctx.ext
                .set_transient_storage(&Key::Fix([1; 32]), Some(vec![42]), false),
            Ok(WriteOutcome::Overwritten(3))
        );
        assert_eq!(
            ctx.ext
                .set_transient_storage(&Key::Fix([2; 32]), Some(vec![48]), true),
            Ok(WriteOutcome::Taken(vec![4, 5, 6]))
        );
        assert_eq!(
            ctx.ext
...
fn get_transient_storage_works() {
    ...
    let code_bob = MockLoader::insert(Call, |ctx, _| {
        if ctx.input_data[0] == 0 {
            assert_eq!(
                ctx.ext
                    .set_transient_storage(storage_key_1, Some(vec![1, 2]), false),
                Ok(WriteOutcome::New)
            );
            assert_eq!(
                ctx.ext
            ...
            assert_eq!(
                ctx.ext
                    .set_transient_storage(storage_key_1, Some(vec![3]), true),
                Ok(WriteOutcome::Taken(vec![1, 2]))
            );
            assert_eq!(
                ctx.ext
                    .set_transient_storage(storage_key_2, Some(vec![]), false),
                Ok(WriteOutcome::New)
            );
        }
        exec_success()
...
fn get_transient_storage_size_works() {
    ...
    let code_hash = MockLoader::insert(Call, |ctx, _| {
        assert_eq!(
            ctx.ext
                .set_transient_storage(storage_key_1, Some(vec![1, 2, 3]), false),
            Ok(WriteOutcome::New)
        );
        assert_eq!(
            ctx.ext
...
fn rollback_transient_storage_works() {
    ...
    let code_bob = MockLoader::insert(Call, |ctx, _| {
        if ctx.input_data[0] == 0 {
            assert_eq!(
                ctx.ext
                    .set_transient_storage(storage_key, Some(vec![1, 2]), false),
                Ok(WriteOutcome::New)
            );
            assert_eq!(
                ctx.ext
            ...
            assert_eq!(
                ctx.ext
                    .set_transient_storage(storage_key, Some(vec![3]), false),
...
fn last_frame_output_is_always_reset() {
    let code_bob = MockLoader::insert(Call, |ctx, _| {
        ...
        let output_revert = || ExecReturnValue {
            flags: ReturnFlags::REVERT,
            data: vec![1],
        };
...
fn immutable_data_access_checks_work() {
    ...
    let instantiator_ch = MockLoader::insert(Call, {
        move |ctx, _| {
            ...
            assert_eq!(
                ctx.ext
                    .set_immutable_data(vec![0, 1, 2, 3].try_into().unwrap()),
                Err(Error::<Test>::InvalidImmutableAccess.into())
            );
...
fn immutable_data_set_overrides() {
    let hash = MockLoader::insert_both(
        ...
        move |ctx, _| {
            assert_eq!(
                ctx.ext.get_immutable_data().unwrap().into_inner(),
                vec![7, 5]
            );
            exec_success()
        },
...
fn block_hash_returns_proper_values() {
    let bob_code_hash = MockLoader::insert(Call, |ctx, _| {
        ...
        assert_eq!(
            ctx.ext.block_hash(U256::from(44)),
            Some(H256::from([2; 32]))
        );
