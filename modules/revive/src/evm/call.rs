...
impl GenericTransaction {
        ...
        CallOf<T>: SetWeightLimit,
    {
        ...
        match (self.chain_id, self.r#type.as_ref()) {
            ...
            (Some(chain_id), ..) => {
                if chain_id != <T as Config>::ChainId::get().into() {
                    log::info!(target: LOG_TARGET, "Invalid chain_id {chain_id:?}");
                    return Err(InvalidTransaction::Call);
                }
            }
            (None, ..) => {
                log::info!(target: LOG_TARGET, "Invalid chain_id None");
                return Err(InvalidTransaction::Call);
            }
        }

        let Some(gas) = self.gas else {
            log::info!(target: LOG_TARGET, "No gas provided");
            return Err(InvalidTransaction::Call);
        };
        ...
        let Some(effective_gas_price) = self.gas_price else {
            log::info!(target: LOG_TARGET, "No gas_price provided.");
            ...
            if dest == RUNTIME_PALLETS_ADDR {
                let call =
                    CallOf::<T>::decode_all_with_depth_limit(MAX_EXTRINSIC_DEPTH, &mut &data[..])
                        .map_err(|_| {
                        log::info!(target: LOG_TARGET, "Failed to decode data as Call");
                        InvalidTransaction::Call
                    })?;

                if !value.is_zero() {
                    log::info!(target: LOG_TARGET, "Runtime pallets address cannot be called with value");
                    return Err(InvalidTransaction::Call);
                }

                crate::Call::eth_substrate_call::<T> {
                    call: Box::new(call),
                    transaction_encoded,
                }
                .into()
        ...
        let weight_limit = {
            ...
            let remaining_fee = {
				...
				log::info!(target: LOG_TARGET, "Not enough gas supplied to cover base and len fee. eth_fee={eth_fee:?} fixed_fee={fixed_fee:?}");
			...
			log::info!(target: LOG_TARGET, "Not enough gas supplied to cover the weight ({:?}) of the extrinsic. remaining_fee_weight: {remaining_fee_weight:?}", info.total_weight(),);
			InvalidTransaction::Payment
		})?;
