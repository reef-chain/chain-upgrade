...
mod benchmarks {
    ...
    fn remove_code() -> Result<(), BenchmarkError> {
        ...
        assert_eq!(
            uploaded.deposit,
            T::Currency::total_balance_on_hold(&caller)
        );
        assert!(<Contract<T>>::code_exists(&hash));
        #[extrinsic_call]
        _(origin, hash);
    ...
    fn seal_ref_time_left() {
        ...
        assert_eq!(
            result.unwrap(),
            runtime.ext().gas_meter().gas_left().ref_time()
        );
    }
    ...
    fn seal_balance_of() {
        ...
        assert_eq!(
            ...
            runtime.ext().balance_of(&address)
        );
    }
    ...
    fn seal_return_data_size() {
        ...
        *runtime.ext().last_frame_output_mut() = ExecReturnValue {
            data: vec![42; 256],
            ..Default::default()
        };
        let result;
    ...
    fn seal_return(n: Linear<0, { limits::code::BLOB_BYTES - 4 }>) {
        ...
        assert!(matches!(
            result,
            Err(crate::wasm::TrapReason::Return(
                crate::wasm::ReturnData { .. }
            ))
        ));
    }
    ...
    fn set_transient_storage_empty() -> Result<(), BenchmarkError> {
        ...
        assert_eq!(
            runtime.ext().get_transient_storage(&key),
    ...
    fn seal_instantiate(i: Linear<0, { limits::code::BLOB_BYTES }>) -> Result<(), BenchmarkError> {
        ...
        let hash = Contract::<T>::with_index(1, WasmModule::dummy(), vec![])?
            .info()?
            .code_hash;
    ...
    fn sha2_256(n: Linear<0, { limits::code::BLOB_BYTES }>) {
        ...
        assert_eq!(
            sp_io::hashing::sha2_256(&input).to_vec(),
            result.unwrap().data
        );
    }
    ...
    fn bn128_pairing(n: Linear<0, { 20 }>) {
        fn generate_random_ecpairs(n: usize) -> Vec<u8> {
            ...
            let mut write = |element: &bn::Fq, offset: &mut usize| {
                element
                    .to_big_endian(&mut buffer[*offset..*offset + 32])
                    .unwrap();
                *offset += 32
            };
    ...
    fn seal_ecdsa_to_eth_address() {
        ...
        assert_eq!(
            &memory[..20],
            runtime.ext().ecdsa_to_eth_address(&pub_key_bytes).unwrap()
        );
    }

    #[benchmark(pov_mode = Measured)]
    fn seal_set_code_hash() -> Result<(), BenchmarkError> {
        let code_hash = Contract::<T>::with_index(1, WasmModule::dummy(), vec![])?
            .info()?
            .code_hash;
