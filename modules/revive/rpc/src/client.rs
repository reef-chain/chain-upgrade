...
impl Client {
    ...
    pub async fn subscribe_and_cache_new_blocks(
        ...
        self.subscribe_new_blocks(subscription_type, |block| async {
            let (signed_txs, receipts): (Vec<_>, Vec<_>) = self
                .receipt_provider
                .insert_block_receipts(&block)
                .await?
                .into_iter()
                .unzip();

            let evm_block = self
                .evm_block_from_receipts(&block, &receipts, signed_txs, false)
                .await;
            self.block_provider
                .update_latest(block, subscription_type)
                .await;
    ...
    > {
        ...
        let res = self
            .rpc_client
            .request("chain_getBlock".to_string(), params)
            .await
            .unwrap();
    ...
    pub async fn trace_transaction(
        ...
        runtime_api
            .trace_tx(block, transaction_index.as_u32(), config.clone())
            .await
    }
    ...
    pub async fn evm_block_from_receipts(
        ...
        let gas_used = receipts
            .iter()
            .fold(U256::zero(), |acc, receipt| acc + receipt.gas_used);
