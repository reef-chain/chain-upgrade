...
pub mod module {
    ...
    pub(crate) type Commitments<T: Config> =
        StorageMap<_, Blake2_128Concat, T::AccountId, CommitmentOf<T>>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn add_funds(
            ...
            let mut commitment =
                Commitments::<T>::get(&origin).ok_or(Error::<T>::NoCommitmentsFound)?;
...
impl<T: Config> Pallet<T> {
    ...
    pub fn proportion_of_era_to_year() -> Perbill {
        PerThing::from_rational(T::EraDuration::get(), 365 * primitives::time::DAYS)
    }
}
