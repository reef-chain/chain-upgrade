...
fn deploy_contract(account: AccountId) -> Result<H160, DispatchError> {
    ...
    if let Event::EVM(module_evm::Event::Created(_, address, ..)) =
...
fn test_evm_module() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            let event = Event::EVM(module_evm::Event::TransferredMaintainer(
                contract,
                bob_address,
            ));
            assert_eq!(last_event(), event);
            ...
            assert_eq!(
                Balances::free_balance(alice_account_id()),
                999999999999986330000000000000000
            );
            assert_eq!(
                Balances::free_balance(bob_account_id()),
                amount(1 * MILLI_REEF)
            ...
            assert_eq!(
                Balances::free_balance(alice_account_id()),
                1009999999999986330000000000000000
            );
            assert_eq!(
                Balances::free_balance(bob_account_id()),
                amount(1 * MILLI_REEF) - amount(10 * MICRO_REEF)
            );
        });
}
